...
/*   Updated: 2023/06/20 11:53:57 by joonhlee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	refresh_unit_time2(t_philo *philo, t_timeval time)
{
	...
	if (t_left_die / philo->share->t_eat > 0)
		t_left_min = 13 * T_OFFSET;
		// t_left_min = philo_min(t_left_die / 2, 11 * T_OFFSET);
	else if (t_left_die * 10 < philo->share->t_eat)
		t_left_min = 2 * T_UNIT;
	else
		t_left_min = 3 * T_OFFSET;
	return (t_left_min);
		// t_left_min = philo_min(t_left_die / 4, 2 * T_OFFSET);
